    ...
    <div class="footerBg">            
        <div class="footer_send">
            <div class="send_message">
                <input type="text" placeholder="请输入" v-model="sendMessage">
            </div>
            <div class="send_submit" @click="sendfor">发 送</div>
        </div>
    </div>
</div>
</body>
    ...
    $(function(){
        ...
        methods:{
            ...
            sendfor:function(){
                ...
                $.ajax({
                  url:'{{route('s_order_addProductAfterSalesMessage')}}',
                  type:'POST',
                  async:true,
                   data:{
                    product_order_id:_this.getQueryVariable('pid'),
                    message:_this.sendMessage
                   },
                   "headers": {
                            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                   },
