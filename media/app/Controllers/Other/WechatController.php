...
class WechatController extends Controller
{
    ...
    /**
     * @Get("/authLogin", as="s_wechat_authLogin")
     */
    public  function authLogin(){

        $code = Curl::post('/weixin/createCode',['return_url'=>route('test_wechat_return')]);
    ...
    /**
     * 开放平台微信网页登录
     * @Get("/login/openweixin/back", as="login_openweixin_back")
    ...
    public function index()
    {
        try {
            ...
            if (!$state || !request('code') || !request('state')) return $this->jump($oauth);
            $user = $oauth->user();
     ...
     * @author: viki
     */
    public function jump($oauth)
    {
        $code = Curl::post('/weixin/createCode');
        $returnUrl = request('return_url', route('s_index_index'));


        $paramer['return_url'] = $returnUrl;
        $paramer['wcode'] = $code['data'];
        $paramer['login_type'] = request('login_type', 1);
        $encrypt = encrypt(json_encode($paramer));

        //$backurl .= '&wcode=' . $code['data'] . '&login_type=' . request('login_type', 1);
        if(strstr( env('WECHAT_OPEN_PLATFORM_WEB_CALLBACK'),'?')){
            $backurl = env('WECHAT_OPEN_PLATFORM_WEB_CALLBACK') . '&encrypt_data=' . $encrypt;
        }else{
            $backurl = env('WECHAT_OPEN_PLATFORM_WEB_CALLBACK') . '?encrypt_data=' . $encrypt;
        }
    ...
    /**
     * 开放平台微信网页登录
     * @Get("/wechat/return", as="test_wechat_return")
     * @param Request $request
     * @return mixed
     * @author: viki
     */
